...
* Error Sub-System
	...
	- Implementation Detail:
		...
		The potential perils of pre_action() with cmd_override == False':
...
To get the response text, web_main calls app_main. app_main is the heart of the app. It recieves user_input from web_app and converts it into output (the app's response text). In the special case of the game's first turn, app_main calls start_me_up which prints a welcome and sets some one-time game variables. But for all subsequent calls to app_main the following flow occurs:
	1) load the object variables from the game's active pickle and reset the output buffer
	...
	3) The interpreted command is checked by validate(). If it is not valid, an error message will be buffered by validate() and no stateful changes to the game will take place. If valid, move count gets incremented and we move on to executing the player's command. 
	4) before executing the game command, app_main now calls the pre_action function. pre_action scans the available machine scope (simplisticaly, the room Burt is in) for machines that have pre_act triggers. If any exist, checks to see if any pre_action Machines are triggered and, if so,  runs those Machines. The pre_action function returns the boolean variable cmd_override to app_main. cmd_override == True is for cases where the pre_action negates the player's command.
	5) If cmd_override is False, app_main now calls cmd_exe to execute the player's command (e.g. "take key" => "Taken"). From the three cases of cmd_exe(), calls disp_score() from the Score subclass.
	6) disp_score() checks whether the player's current command matches any of the score conditions in score_dict. If so, score is increased and an update to the player is buffered.
	7) app_main now calls the post_action function which determines if the player's command (for example, pulling a lever) triggers a post_action Machine. If so, the Machine is run. post_act() also calls disp_score() to see if Machine results have impacted the player's score.
	8) if gs.end.is_end == True then app_main calls gs.end.disp_end() method to buffer the end of game text
	9) if not gs.end.is_end , app_main() calls auto_action() which is essentially the 'game's turn' to take independent actions before player's next turn (e.g. a Creature independently moving).
	10) app_main() saves the updated objects to the save pickle and resturns 'output' and 'end_of_game' to web_main

The key take-away from the app_main flow is that, both before and after the player's command execution call, the game gets a "response turn" and then after all responses, the game gets its "own turn". These ad-hoc responsive and pro-active game actions are what the Machine construct enables.
...
Triggers:
Triggers are the Player Commands or Switches that can start Machines. The information about a Trigger lives in the Machine class itself - a given Switch has no "knowledge" that it's the trigger for a machine and multiple Machines could have the exact same Trigger (this makes sense when you think about Player Commands being triggers).
...
The Switch class is implemented as a MixIn that is combined with ViewOnly and other object classes to include Switch-specific attributes. Each specific type of switch (ButtonSwitch, SpringSliderSwitch, LeverSwitch) contains the methods needed to act on it (e.g. push or pull).


Conditions:
Triggers are what start Machines but Conditions determine what happens when they run. For example, if in the Entrance, Burt goes East off of the drawbridge, then there are three possible conditions: 1) Burt doesn't have a weapon in his hand, 2) Burt does have a weapon and has not gotten the Royal Crown yet, 3) Burt has a weapon and has already gotten the Royal Crown. Each of these conditions will then be associated with a different Result. A Machine's Conditions are limited to examination of Game State, Switches that are associated with the Machine, and the Machine's State.
...
Note: control_panel has since been chenged to class ContainerFixedSimpleMach and the controls are now "on" on the Control Panel surface rather than in the Room features - but the general example still holds.
...
Note: During the cleesh engine 3.8.0 (build 11) coding, the Warning class was fully refactored. The main objective here was code re-use and inheritance from the ProtoMachMixIn and TrigMixIn classes. In doing so, mach_state was re-purposed for warn_count. Warning also inherits the is_enabled attribute and sets it to false after a variable warning has incremented to mach_state == warn_max. 
...
The intent is that timers are "dumb". They are not triggered by player commands or switches - instead they are started or reset via the timer.start() and timer.reset() methods - typically by complex machines. Once started the counter's only job is to count up until it reaches it's maximum. To enable this, the counter has attributes of 'is_active' (boolean), 'mach_state' (formerly 'timer_count'), and 'timer_max'. The timer "done" state (referred to as 'dinging') is inferred from (timer.is_active and mach_state == timer_max) by the timer.is_dinging() method. The mach_run() method is called when the timer is active. Its primary job is to increment timer_count and reset the timer after it has been dinging for a turn (mach_run is called via auto_action).

Although Timers never directly trigger actions (this is left to machines which can use Timer attributes in their Conditions), one typically does want to let the player know that a timer is running. To this end, Timers have the innate ability to send messages to the player on each Timer interval. For each turn it is active, the timer checks its alert_anchor to see if the hero should be able to detect the timer alert. If so, it tries to buffer a constant message if one exists (timer_name + "_0"). Failing this, it attempts to buffer a variable message if one exists (timer_name + "_" + mach_state).

Note: During the cleesh engine 3.8.0 (build 11) coding, the Timer class was fully refactored. The core refactyoring goal was to enable inheritance between Timer and other Mach flavors. timer_done and message-type were also eliminated / converted to methods during this refactor. In terms of core functionality, perhpas the most significant overhaul was the update to how timers are reset and the movement of Mach.trigger_type from pre_act_timer => auto_act_timer. This codified the idea that all timer-based events should happen within the auto_action() phase of the game.
