...
def hand_mgmt(case, word_lst, gs):
    verb_str = word_lst[0]
    # 'drop' and 'eat' to be added
    if verb_str in ['wear'] and not gs.core.hero.chk_in_hand(word_lst[1]) and gs.core.hero.chk_in_bkpk(word_lst[1]):
        do_noun_obj = word_lst[1]
        gs.core.hero.put_in_hand(do_noun_obj, gs)
        gs.core.hero.bkpk_lst_remove(do_noun_obj)
    # 'drop', 'stow', and 'eat' to be added
    if verb_str in [] and not gs.core.hero.chk_in_hand(word_lst[1]) and gs.core.hero.chk_is_worn(word_lst[1]):
        do_noun_obj = word_lst[1]
        gs.core.hero.put_in_hand(do_noun_obj, gs)
