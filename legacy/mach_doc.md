...
IDEA:
	...
	- conditions = switch values or gs values
	- results = gs values, machine_state, and cmd_override value
...
- For inter_obj auto case, need scope_list in gs so that inter_obj always gets a turn - even when not in room
...
- Machine attributes [entrance east / west example]
		...
		result_lst =>
			[die_in_moat, => buffer message, gs.game_ending = 'death', return override == True
			moat_get_crown, => buffer message, gs.hand_lst_append(crown), got_crown = True, return override == True
